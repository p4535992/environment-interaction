...
export class EnvironmentInteractionPlaceableConfig {
    ...
    */
		...
		const enviromentChecked = EnvironmentInteractionPlaceableConfig.getEnviroments(
			...
			: "";
		//@ts-ignore
		const actorsOrderByName = <Actor[]>game.actors?.contents.sort((a, b) => a.name.localeCompare(b.name));
	...
	static _applyEnviroments(document, updateData) {
		...
		for (const propertyName of properties) {
          ...
          if (actor) {
            // Set actor reference
            // setProperty(actor, propertyNameOr, eis);
            setProperty(actor.token, propertyNameOr, eis);
	...
	//     }
	//     // TODO MAKE THIS SYNC BETWEEN TOKEN AND PROTOTYPE TOKEN BETTER
	//     if (noPropertyFound && document.flags[CONSTANTS.MODULE_NAME]) {
	...
	//             setProperty((<Actor>t.actor).token, `flags.${CONSTANTS.MODULE_NAME}.${Flags.environmentToken}`, document.getFlag(CONSTANTS.MODULE_NAME, Flags.environmentToken));
	//             setProperty((<Actor>t.actor).token, `flags.${CONSTANTS.MODULE_NAME}.${Flags.environmentTokenRef}`, document.getFlag(CONSTANTS.MODULE_NAME, Flags.environmentTokenRef));
	//           }
	//         });
	//       }
	//     }
	...
	static _validateEnviroments(inEnviroments, inFunctionName) {
		if (!(typeof inEnviroments === "string" || typeof inEnviroments === "string" || Array.isArray(inEnviroments))) {
			throw new Error(
				`${CONSTANTS.MODULE_NAME} | ${inFunctionName} | inEnviroments must be of type string or array`
			);
		}
		...
		providedEnviroments.forEach((t) => {
			if (typeof t !== "string")
				throw new Error(
					`${CONSTANTS.MODULE_NAME} | ${inFunctionName} | enviroments in array must be of type string`
				);
		});
