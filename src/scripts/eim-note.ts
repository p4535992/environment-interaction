	...
	_addAceLibEditorToElement(
		...
		configElement.find(`.ei-macro-editor-expand-${entityFieldId}-${flagRef}`).on("click", (event) => {
			...
			if (configElement.find(`.ei-macro-editor-${entityFieldId}-${flagRef}`).hasClass("fullscreen")) {
				...
				configElement
					.find(`.ei-macro-editor-expand-${entityFieldId}-${flagRef}`)
					.prop("title", "Expand Editor");
				configElement
	...
	async _updateObject(event, formData) {
		if (game.user?.isGM) {
			...
			if (useasmacro != null && useasmacro != undefined) {
				await this.entity.setFlag(
					CONSTANTS.MODULE_NAME,
					EnvironmentInteractionFlags.notesuseasmacro,
					useasmacro
				);
			} else {
			...
			if (explicitdc != null && explicitdc != undefined) {
				await this.entity.setFlag(
					CONSTANTS.MODULE_NAME,
					EnvironmentInteractionFlags.notesexplicitdc,
					explicitdc
				);
			} else {
