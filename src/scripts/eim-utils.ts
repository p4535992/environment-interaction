...
export const executeEIMacro = function (item: Item, macroFlag: string, ...args: any[]): any {
	...
	if (matchAllTags) {
		for (const matchTag of matchAllTags) {
			if (matchTag) {
				...
				//@ts-ignore
				macroContent = (<Macro>game.macros?.getName(macroName)).command;
			}
		}
	}
	...
	// const fn2 = new Function('item', 'speaker', 'actor', 'token', 'character', 'event', 'args', macro.command);
...
export async function rollSimple(item, extraContents) {
	const img = item.img || "icons/svg/d20-highlight.svg";
