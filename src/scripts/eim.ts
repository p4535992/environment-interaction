...
export class EnvironmentInteraction {
	...
	static _canViewToken(wrapped, ...args) {
		...
		// const isEi =
		//   token.actor && token.actor.token
		//     ? getProperty(token.actor.token, `flags.${CONSTANTS.MODULE_NAME}.${Flags.environmentToken}`)
		//     : // ? token.document.getFlag(CONSTANTS.MODULE_NAME,Flags.environmentToken)
		//       false;
		const isEi =
			token.document.getFlag(CONSTANTS.MODULE_NAME, EnvironmentInteractionFlags.environmentToken) ?? false;
	...
	static _DoorControlPrototypeOnMouseDownHandler(wrapped, ...args) {
		...
		// const isDoor = wall.door > 0;
		const wall = <Wall>canvas.walls?.placeables.find((x) => {
			return x.id == doorControl.wall.id;
		});
		const isEi =
			wall.document.getFlag(CONSTANTS.MODULE_NAME, EnvironmentInteractionFlags.environmentToken) ?? false;
		if (!isEi) {
			return wrapped(...args);
		} else {
	...
	static _NotePrototypeOnClickLeftHandler = async function (wrapped, ...args) {
		...
		const isEi =
			note.document.getFlag(CONSTANTS.MODULE_NAME, EnvironmentInteractionFlags.environmentToken) ?? false;
		if (!isEi) {
			return wrapped(...args);
		} else {
		...
		environmentActorExternal?.items
			.filter((environmentItemToCheck) => {
				if (environmentItemToCheck.getFlag(CONSTANTS.MODULE_NAME, EnvironmentInteractionFlags.notesuseei)) {
					if (
						environmentItemToCheck.getFlag(
							CONSTANTS.MODULE_NAME,
							EnvironmentInteractionFlags.notescondition
						)
					) {
						const result = executeEIMacro(
							environmentItemToCheck,
							EnvironmentInteractionFlags.notescondition
						);
						if (result) {
							return result;
		...
		const render = (html) => {
			html.on("click", `button.ei-info`, async (event) => {
				...
				let contentInfo = <string>(
					environmentItem.getFlag(CONSTANTS.MODULE_NAME, EnvironmentInteractionFlags.notesinfo)
				);
				if (!contentInfo) {
					contentInfo = "No info provided";
				}
				const d = new Dialog({
					title: <string>environmentItem.name,
					content: contentInfo,
					buttons: {},
					default: "",
					render: (html) => {
				...
				html.on("click", `button.ei-flex-container`, async (event) => {
					...
					customInfo.environmentPlaceableObjectTYPE = <string>(
						environmentPlaceableObject.document.documentName
					);
					...
					// try {
					//@ts-ignore
					// const action = <string>ownedItem.getFlag(CONSTANTS.MODULE_NAME,Flags.notesdetail); //ownedItem.actionType; //$(button).data('action');
					...
					try {
						...
						const useItemMacro = <boolean>(
							environmentItem.getFlag(
								CONSTANTS.MODULE_NAME,
								EnvironmentInteractionFlags.notesuseitemmacro
							)
						);
						...
						if (useItemMacro) {
							...
							//@ts-ignore
							if (interactorItemTmp.flags.itemacro?.macro && isItemMacroModuleActive()) {
								...
								if (isSystemItemMacroSupported() && interactorItemTmp.hasMacro()) {
									...
									warn(
										`No macro found for the integration with 'item macro' for launch a macro`,
										true
									);
									return;
								}
						...
						if (useAsMacro) {
							...
							const macroArgs = environmentItem.getFlag(
								CONSTANTS.MODULE_NAME,
								EnvironmentInteractionFlags.notesargs
							);
							...
							if (result >= explicitDC) {
								...
								const macroContentFailure = <string>(
									environmentItem.getFlag(
										CONSTANTS.MODULE_NAME,
										EnvironmentInteractionFlags.notesfailure
									)
								);
						...
						if (myRequestArray.length == 0) {
							warn(
								`The label event setted for the environment interaction is invalid '${REQUEST_LABEL}'`,
								true
							);
							return;
						}
						...
						//@ts-ignore
						const labelOrDcReq = <string>myRequestArray[2]?.trim() ?? interactorItemTmp.system.source;
						...
						switch (environmentTypeReq) {
							...
							case ENVIRONMENT_TYPE.DICE: {
								...
								// const result = <number>await roll.total ?? 0;
								let result = 0;
								const formula = macroNameOrTypeReq.includes("data")
									? macroNameOrTypeReq.replace(/data\./g, "@")
									: macroNameOrTypeReq;
								const data = interactorToken.document.actor
									? interactorToken.document.actor.getRollData()
									: {};
							...
							case ENVIRONMENT_TYPE.MACRO: {
								...
								if (macroNameOrTypeReq) {
									...
									const macro = <Macro>(
										(game.macros?.getName(macroName) || game.macros?.get(macroName))
									);
									if (!macro) {
							...
							case ENVIRONMENT_TYPE.ATTACK: {
								...
								if (isSystemTokenActionHUDSupported() && isTokenActionHudActive()) {
									...
									const some = await getTokenActionHUDRollHandler().doHandleActionEvent(
										event,
										payload
									);
									log(some);
							...
							case ENVIRONMENT_TYPE.ABILITY: {
								...
								if (isSystemMonkTokenBarSupported() && isMonkTokensBarModuleActive()) {
									...
									options.flavor = <string>environmentItem.name;
									options.request = macroNameOrTypeReq;
									options.dc = dcReq;

									const groupsReq = labelOrDcReq?.trim()
										? Array.from(labelOrDcReq?.trim().split(","))
										: [];
									...
									if (options.request.includes(":")) {
										...
										warn(
											i18n(
												`${CONSTANTS.MODULE_NAME}.interactWithEnvironment.noValidRequestWarn`
							...
							case ENVIRONMENT_TYPE.SKILL: {
								...
								if (isSystemMonkTokenBarSupported() && isMonkTokensBarModuleActive()) {
									log(`Try the Monk Tokens Bar integration`);
									//const save = environmentItem.system.save.ability;
									...
									if (options.request.includes(":")) {
										if (options.request.includes(",")) {
											...
											request1.request = req1; //'save:'+ environmentItem.system.save.ability;

											const request0: any = new Object();
											request0.token = interactorToken.id;
											request0.request = req0; //'save:'+ interactorItem.system.save.ability;
											...
											const some = await getMonkTokenBarAPI().requestContestedRoll(
												request1,
												request0,
												options
											);
											...
											const some = await getMonkTokenBarAPI().requestRoll(
												[interactorToken],
												options
											);
											log(some);
						...
						if (interactorItemTmp && interactorItemTmp?.id) {
							...
							if (found) {
								if (interactorItemTmp.actor?.id == interactorToken.actor?.id) {
									...
									await wait(secondsOfDelayForDeletion * 1000); // SECONDS TO MILLISECONDS
