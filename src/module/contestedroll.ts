export class ContestedRoll {
  ...
  static async returnRoll(id, roll, actor, rollmode) {
    ...
    if (roll != undefined) {
      let finishroll;
      if (game.dice3d != undefined && roll instanceof Roll) {
        // && !fastForward) {
        let whisper =
          rollmode == 'roll'
            ? null
            : ChatMessage.getWhisperRecipients('GM').map((w) => {
                return w.id;
              });
        if (rollmode == 'gmroll' && !game.user.isGM) whisper.push(game.user.id);
      ...
      if (sound != undefined) AudioHelper.play({ src: sound });
    ...
    if (actor != undefined) {
      if (requesttype == 'dice') {
        ...
        return ContestedRoll.rollDice(request).then((roll) => {
          return returnRoll(roll);
        });
      } else {
        if (MonksTokenBar.system._supportedSystem) {
          //game.system.id == 'dnd5e' || game.system.id == 'sw5e' || game.system.id == 'pf1' || game.system.id == 'pf2e' || game.system.id == 'tormenta20' || game.system.id == 'ose' || game.system.id == 'sfrpg') {
          return MonksTokenBar.system.roll(
            { id: data.id, actor: actor, request: request, requesttype: requesttype, fastForward: fastForward },
            function (roll) {
              return ContestedRoll.returnRoll(data.id, roll, actor, rollmode);
            },
            e,
          );
        } else ui.notifications.warn(i18n('environment-interaction.UnknownSystem'));
      }
    }
  ...
  static async onRollAbility(ids, message, fastForward = false, e) {
    ...
    Promise.all(promises).then((response) => {
      ...
      if (!game.user.isGM) {
        let responses = response.map((r) => {
          return { id: r.id, roll: r.roll };
        });
        game.socket.emit(
          MonksTokenBar.SOCKET,
          {
    ...
    for (let update of updates) {
      if (update != undefined) {
        ...
        if (update.roll) {
          ...
          $(tooltip)
            .hide()
            .insertAfter($('.item[data-item-id="' + update.id + '"] .item-row', content));
          ...
          $('.item[data-item-id="' + update.id + '"] .item-row .roll-controls', content).append(`<div class="dice-total flexrow noselect"><div class="dice-result noselect">Error!</div ></div >`);
          msgtoken.reveal = true;
          msgtoken.error = true;
          flags['token' + update.id] = msgtoken;
        }

        if (update.finish != undefined) promises.push(update.finish);
      }
    }
  ...
  static async finishRolling(updates, message) {
    ...
    if (!game.user.isGM) {
      let response = updates.filter((r) => {
        return r.userid == game.userId;
      });
      if (response.length) {
        game.socket.emit(MonksTokenBar.SOCKET, {
          msgtype: 'finishroll',
          type: 'contestedroll',
          senderId: game.user.id,
          response: response,
  ...
  static async checkResult(message) {
    ...
    if (Object.values(flags).length > 0) message.update({ flags: { 'environment-interaction': flags } });
...
Hooks.on('controlToken', (token, delta) => {
  if (MonksTokenBar && MonksTokenBar.system) {
    ...
    if (game.user.isGM && delta === true && contestedroll != undefined && contestedroll._state != -1) {
      if (contestedroll.item0.token == undefined) contestedroll.item0.token = token;
      else if (contestedroll.item1.token == undefined) contestedroll.item1.token = token;
...
Hooks.on('renderChatMessage', (message, html, data) => {
  ...
  if (svgCard.length !== 0) {
    ...
    if (game.user.isGM) html.find('.player-only').remove();
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (msgtoken) {
        ...
        if (game.user.isGM || actor.isOwner) $('.item-image', item).on('click', $.proxy(ContestedRoll._onClickToken, this, msgtoken.id));
        $('.item-roll', item)
          .toggle(msgtoken.roll == undefined && (game.user.isGM || (actor.isOwner && rollmode != 'selfroll')))
          .click($.proxy(ContestedRoll.onRollAbility, this, msgtoken.id, message, false));
        ...
        if (msgtoken.roll != undefined && msgtoken.roll.class.includes('Roll')) {
          ...
          if (!msgtoken.reveal || (rollmode == 'blindroll' && !game.user.isGM)) $('.dice-result', item).html(!msgtoken.reveal ? '...' : '-');
          if (!msgtoken.reveal && game.user.isGM) $('.dice-result', item).on('click', $.proxy(ContestedRoll.finishRolling, ContestedRoll, [msgtoken.id], message));
          //if (showroll && !msgtoken.rolling && $('.dice-tooltip', item).is(':empty')) {
          //    let tooltip = await roll.getTooltip();
          //    $('.dice-tooltip', item).empty().append(tooltip);
          //}
          if (game.user.isGM) $('.roll-result', item).click($.proxy(ContestedRoll.setRollSuccess, this, msgtoken.id, message, true));
      ...
      $('.select-all', html).on(
        'click',
        $.proxy(MonksTokenBar.selectActors, MonksTokenBar, message, (ti) => {
          return ti;
        }),
      );
      $('.select-saved', html).on(
        'click',
        $.proxy(MonksTokenBar.selectActors, MonksTokenBar, message, (ti) => ti?.passed === 'won'),
      );
      $('.select-failed', html).on(
        'click',
        $.proxy(MonksTokenBar.selectActors, MonksTokenBar, message, (ti) => ti?.passed === 'failed'),
      );
