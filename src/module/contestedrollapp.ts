export class ContestedRollApp extends Application {
  constructor(item0, item1, options = {}) {
    ...
    this.hidenpcname =
      (options?.hidenpcname != undefined ? options?.hidenpcname : null) ||
      (game.user.getFlag('environment-interaction', 'lastmodeHideNPCName') != undefined ? game.user.getFlag('environment-interaction', 'lastmodeHideNPCName') : null) ||
      false;
  }
  ...
  async request() {
    if (this.item0.token != undefined && this.item1.token != undefined) {
      ...
      let modename =
        rollmode == 'roll'
          ? i18n('environment-interaction.PublicRoll')
          : rollmode == 'gmroll'
          ? i18n('environment-interaction.PrivateGMRoll')
          : rollmode == 'blindroll'
          ? i18n('environment-interaction.BlindGMRoll')
          : i18n('environment-interaction.SelfRoll');
      let requestdata = {
        rollmode: rollmode,
        modename: modename,
        tokens: tokens,
      ...
      for (let i = 0; i < tokens.length; i++) requestdata['token' + tokens[i].id] = tokens[i];
          ...
          if (item.token.actor != undefined) {
            for (var key in item.token.actor.data.permission) {
              if (key != 'default' && item.token.actor.data.permission[key] >= CONST.ENTITY_PERMISSIONS.OWNER) {
                if (chatData.whisper.find((t) => t == key) == undefined) chatData.whisper.push(key);
              }
            }
          }
        }
      ...
      if (setting('request-roll-sound-file') != '' && rollmode != 'selfroll') AudioHelper.play({ src: setting('request-roll-sound-file') }, true);
      this.close();
    } else ui.notifications.warn(i18n('environment-interaction.RequestActorMissing'));
  }
